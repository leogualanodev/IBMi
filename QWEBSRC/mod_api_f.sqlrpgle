...
DCL-PROC GET_EMPLOYE SERIALIZE EXPORT;
  ...
  BegSr GET_SR;
    ...
    SELECT JSON_ARRAYAGG(JSON_OBJECT(
      ...
      KEY 'AGE' VALUE AGE,
      KEY 'POSTE' VALUE POSTE
      ABSENT ON NULL RETURNING CLOB FORMAT JSON)
      ORDER BY 'ID' RETURNING CLOB(64K) FORMAT JSON) INTO :JSON:NIn_CLOB
      FROM EMPLOYE;
...
DCL-PROC POST_EMPLOYE SERIALIZE EXPORT;
  ...
  BegSr POST_SR;
    ...
    SET
    :NOM=JSON_VALUE(
    :JSON FORMAT JSON, '$.NOM'
    RETURNING VARCHAR(50)),
    :PRENOM=JSON_VALUE(
    :JSON FORMAT JSON, '$.PRENOM'
    RETURNING VARCHAR(50)),
    :AGE=JSON_VALUE(
    :JSON FORMAT JSON, '$.AGE'
    RETURNING DECIMAL(3, 0)),
    :POSTE=JSON_VALUE(
    :JSON FORMAT JSON, '$.POSTE'
    RETURNING VARCHAR(50));
